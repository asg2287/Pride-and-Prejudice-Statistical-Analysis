...
riemann_bars_A <- global_integration %>% filter(Character %in% c("Elizabeth", "Mr. Darcy", "Jane", "Mrs. Bennet", "Mr. Bingley", "Mr. Wickham", "Lydia", "Mr. Collins"))
top_12_characters_A <- raw_data %>% group_by(Character) %>% summarise(Total = sum(A)) %>% slice_max(Total, n = 12) %>% pull(Character)


fig1_1 <- ggplot(filter(raw_data, Character %in% top_12_characters_A), aes(x = Absolute_Chapter, y = A)) +
  geom_area(fill = "#D4E6F1", alpha = 0.7, color = "#5499C7", size = 0.3) +
  facet_wrap(~ factor(Character, levels = top_12_characters_A), ncol = 2, scales = "free_y") +
  scale_x_continuous(limits = c(1, 61), breaks = c(1, 10, 21, 31, 41, 51, 61)) +
  labs(title = "Figure 1.1: Action Vector Trajectories", subtitle = "Continuous Riemann Distribution of Action (A)", x = "Chapters 1-61", y = "Action Intensity (A)") +
  theme_manuscript()
ggsave("FIGURE_1_1_A_VECTOR_TRAJECTORIES.png", plot = fig1_1, width = 9.5, height = 11, units = "in", dpi = 300)


fig1_2_bars_data <- riemann_bars_A %>% pivot_longer(cols = -Character, names_to = "Dimension", values_to = "Volume")
...
fig1_3 <- ggplot(raw_data %>% filter(Character %in% top_12_characters_A), aes(x = Absolute_Chapter, y = A, fill = reorder(Character, -A, FUN = sum))) +
  geom_area(position = "stack", alpha = 0.85, color = "white", size = 0.1) +
  scale_fill_brewer(palette = "Spectral", name = "Action Volume Hierarchy") +
  labs(title = "Figure 1.3: Geometric Mass of Narrative Action", subtitle = "Cumulative Riemann Integration of Action Across 61 Chapters", x = "Chapters 1-61", y = "Stacked Action Magnitude") +
  theme_manuscript()
ggsave("FIGURE_1_3_GEOMETRIC_MASS_OF_A.png", plot = fig1_3, width = 10, height = 6.5, units = "in", dpi = 300)

fig1_4 <- ggplot(raw_data %>% filter(Character %in% c("Elizabeth", "Jane", "Mr. Darcy", "Mrs. Bennet")), aes(x = Absolute_Chapter, y = A, color = Character, fill = Character)) +
  ...
  labs(title = "Figure 1.4: Trajectory of Character Action (A)", x = "Chapters 1-61", y = "Action Score") +
  theme_manuscript() + theme(legend.position = "none")
ggsave("FIGURE_1_4_TRAJECTORY_OF_A.png", plot = fig1_4, width = 9.0, height = 8, units = "in", dpi = 300)


fig1_5 <- ggplot(raw_data %>% filter(Character %in% c("Elizabeth", "Mr. Darcy")), aes(x = Absolute_Chapter, y = A, color = Character)) +
  ...
  labs(title = "Figure 1.5: Macro-Temporal Plot Dynamics: Elizabeth and Darcy", subtitle = "Direct Covariance of Action Across the 61 Chapters", x = "Chapters 1-61", y = "Action Intensity Metric") +
  theme_manuscript()
ggsave("FIGURE_1_5_A_COVARIANCE.png", plot = fig1_5, width = 9.5, height = 5.5, units = "in", dpi = 300)

v_a_mean  <- mean(riemann_bars_A$Action, na.rm = TRUE)
v_d2_mean <- mean(riemann_bars_A$`Global Anomaly Score (D²)`, na.rm = TRUE)

fig1_6 <- ggplot(riemann_bars_A, aes(x = Action, y = `Global Anomaly Score (D²)`, label = Character)) +
  geom_vline(xintercept = v_a_mean, linetype = "dashed", color = "grey60", size = 0.5) +
  geom_hline(yintercept = v_d2_mean, linetype = "dashed", color = "grey60", size = 0.5) +
  ...
  labs(title = "Figure 1.6: Character Topology Map", 
       x = "Accumulated Action (A)", 
       y = "Global Anomaly Weight (D²)") +
  theme_manuscript()
ggsave("FIGURE_1_6_A_TOPOLOGY_MAP.png", plot = fig1_6, width = 8.5, height = 7, units = "in", dpi = 300)
