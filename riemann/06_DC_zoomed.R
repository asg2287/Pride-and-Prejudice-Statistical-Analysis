...
fig1_6_dc_full <- ggplot(plot_data_full, aes(x = `Action Volume`, y = `Discussion of Character Volume`, label = Character)) +
  ...
  labs(title = "Figure 1.6: Character Topology Map (Excluding Primary Protagonists)", subtitle = "Sub-Protagonist and Minor Character Discussion of Character Profile", x = "Accumulated Action Volume", y = "Accumulated Discussion of Character Volume") +
  theme_manuscript()

ggsave(file.path(home_dir, "FIGURE_1_6_DC_TOPOLOGY_MAP_CLEAN.png"), plot = fig1_6_dc_full, width = 10, height = 7, dpi = 300)


max_zoom_dc_x <- quantile(plot_data_full$`Action Volume`, 0.75, na.rm = TRUE)
max_zoom_dc_y <- quantile(plot_data_full$`Discussion of Character Volume`, 0.75, na.rm = TRUE)
plot_data_dc_zoomed <- plot_data_full %>% filter(`Action Volume` <= max_zoom_dc_x, `Discussion of Character Volume` <= max_zoom_dc_y)

fig1_6_dc_zoomed <- ggplot(plot_data_dc_zoomed, aes(x = `Action Volume`, y = `Discussion of Character Volume`, label = Character)) +
  geom_point(aes(size = `Global Anomaly Score (D²)`, color = `Global Anomaly Score (D²)`), alpha = 0.75) +
  ...
  scale_color_gradient(low = "#1F618D", high = "#E67E22") + 
  coord_cartesian(xlim = c(0, max_zoom_dc_x), ylim = c(0, max_zoom_dc_y)) +
...
ggsave(file.path(home_dir, "FIGURE_1_6_DC_TOPOLOGY_MAP_ZOOMED.png"), plot = fig1_6_dc_zoomed, width = 10, height = 7, dpi = 300)

message("--- Discussion of Character (DC) Topology Maps Generated Successfully ---")
