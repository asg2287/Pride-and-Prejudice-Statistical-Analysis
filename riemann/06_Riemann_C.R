...
top_12_characters_C <- raw_data %>%
  group_by(Character) %>%
  summarise(Total_Imp = sum(C, na.rm = TRUE)) %>%
  arrange(desc(Total_Imp)) %>%
  slice_max(Total_Imp, n = 12) %>%
  pull(Character)

fig1_1_data <- raw_data %>%
  filter(Character %in% top_12_characters_C)

fig1_1 <- ggplot(fig1_1_data, aes(x = Absolute_Chapter, y = C)) +
  geom_area(fill = "#F3D1D1", alpha = 0.7, color = "#A35C5C", size = 0.3) +
  facet_wrap(~ factor(Character, levels = top_12_characters_C), ncol = 2, scales = "free_y") +
  scale_x_continuous(limits = c(1, 61), breaks = c(1, 10, 21, 31, 41, 51, 61)) +
  labs(title = "Figure 1.1: Communication Vector Trajectories",
       subtitle = "Continuous Riemann Volume Distribution of Communication Across the Top 12 Most Implicated Characters",
       x = "Chapters 1-61", 
       y = "Communication Intensity Metric (C)") +
  theme_manuscript()

ggsave("FIGURE_1_1_6D_VECTOR_TRAJECTORIES.png", plot = fig1_1, width = 9.5, height = 11, units = "in", dpi = 300)
...
character_ranks_C <- raw_data %>%
  group_by(Character) %>%
  summarise(Total_Vol = sum(C, na.rm = TRUE)) %>%
  arrange(desc(Total_Vol))

miss_bingley_index <- which(character_ranks_C$Character == "Miss Bingley")

fig1_3_targets <- if(length(miss_bingley_index) > 0) {
  character_ranks_C$Character[1:(miss_bingley_index - 1)]
} else {
  character_ranks_C$Character
}

fig1_3_data <- raw_data %>%
  filter(Character %in% fig1_3_targets)

fig1_3 <- ggplot(fig1_3_data, aes(x = Absolute_Chapter, y = C, fill = reorder(Character, -C, FUN = sum))) +
  geom_area(position = "stack", alpha = 0.85, color = "white", size = 0.1) +
  scale_fill_brewer(palette = "Spectral", name = "Communication Volume Hierarchy") +
  scale_x_continuous(limits = c(1, 61), breaks = c(1, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56, 61), expand = c(0,0)) +
  labs(title = "The Geometric Mass of Communication Importance",
       subtitle = "Visualising Total Cumulative Riemann Integration Communication Volume Across the 61-Chapter Timeline",
       caption = "Figure 1.3: The Geometric Mass of Narrative Importance: Total Cumulative Stacked Riemann Integration Communication Volume Across the 61-Chapter Timeline.",
       x = "Absolute Chronological Timeline (Chapters 1-61)", 
       y = "Stacked Communication Vector Magnitude (C)") +
  theme_manuscript() +
  theme(legend.position = "right")

ggsave("FIGURE_1_3_GEOMETRIC_MASS_OF_NARRATIVE_IMPORTANCE.png", plot = fig1_3, width = 10, height = 6.5, units = "in", dpi = 300)
...
fig1_4 <- ggplot(fig1_4_data, aes(x = Absolute_Chapter, y = C, color = Character, fill = Character)) +
  ...
  labs(title = "Figure 1.4: The Trajectory of Character Communication Over Time",
       x = "Chapters 1-61", 
       y = "Communication Score (C)") +
  theme_manuscript() +
  theme(legend.position = "none")

ggsave("FIGURE_1_4_TRAJECTORY_OF_CHARACTER_COMMUNICATION.png", plot = fig1_4, width = 9.0, height = 8, units = "in", dpi = 300)

fig1_5_data <- raw_data %>%
  mutate(Character = str_trim(Character)) %>%
  filter(Character %in% c("Elizabeth", "Mr. Darcy"))

fig1_5 <- ggplot(fig1_5_data, aes(x = Absolute_Chapter, y = C, color = Character)) +
  geom_line(size = 1.2) +
  ...
  labs(title = "Figure 1.5: Macro-Temporal Communication Realignment: Elizabeth and Darcy",
       subtitle = "Direct Covariance of Communication Across the 61 Chapters",
       x = "Chapters 1-61", 
       y = "Communication Intensity Metric (C)") +
  theme_manuscript() +
  theme(legend.position = "right")

ggsave("FIGURE_1_5_ELIZABETH_DARCY_COMMUNICATION_COVARIANCE.png", plot = fig1_5, width = 9.5, height = 5.5, units = "in", dpi = 300)

v_a_mean <- mean(riemann_bars_1_2$Action, na.rm = TRUE)
v_c_mean <- mean(riemann_bars_1_2$Communication, na.rm = TRUE)

fig1_6 <- ggplot(riemann_bars_1_2, aes(x = Action, y = Communication, label = Character)) +
  geom_vline(xintercept = v_a_mean, linetype = "dashed", color = "grey60", size = 0.5) +
  geom_hline(yintercept = v_c_mean, linetype = "dashed", color = "grey60", size = 0.5) +
...
ggsave("FIGURE_1_6_CHARACTER_TOPOLOGY_MAP.png", plot = fig1_6, width = 8.5, height = 7, units = "in", dpi = 300)

cat("\n[Execution Complete]: All 6 localized figures generated successfully.\n")
