...
global_integration <- raw_data %>%
  ...
  summarise(
    ...
    `Description by Narrator Volume`    = pracma::trapz(Absolute_Chapter, DN),
...
v_dn_mean  <- mean(global_integration$`Description by Narrator Volume`, na.rm = TRUE)


plot_data_full <- global_integration %>%
  filter(!Character %in% c("Elizabeth", "Mr. Darcy"))

fig1_6_act_dn_full <- ggplot(plot_data_full, aes(x = `Action Volume`, y = `Description by Narrator Volume`, label = Character)) +
  geom_vline(xintercept = v_act_mean, linetype = "dashed", color = "grey60", linewidth = 0.5) +
  geom_hline(yintercept = v_dn_mean, linetype = "dashed", color = "grey60", linewidth = 0.5) +
  ...
  labs(
    ...
    subtitle = "Sub-Protagonist and Minor Character Narrator Description Profile",
    x     = "Accumulated Action Volume (Plot Mechanics)",
    y     = "Accumulated Description by Narrator Volume (Narratorial Scrutiny)"
  ) +
  theme_manuscript() + 
  theme(legend.position = "right")

output_full <- file.path(home_dir, "FIGURE_1_6_DN_TOPOLOGY_MAP_CLEAN.png")
ggsave(output_full, plot = fig1_6_act_dn_full, width = 10, height = 7, units = "in", dpi = 300)
...
plot_data_zoomed <- global_integration %>%
  filter(
    ...
    `Action Volume` <= 35,
    `Description by Narrator Volume` <= 12
  )

fig1_6_act_dn_zoomed <- ggplot(plot_data_zoomed, aes(x = `Action Volume`, y = `Description by Narrator Volume`, label = Character)) +
  geom_point(aes(size = `Global Anomaly Score (D²)`, color = `Global Anomaly Score (D²)`), alpha = 0.75) +
  geom_text_repel(
    family              = "serif", 
    size                = 3.2, 
  ...
  labs(
    ...
    subtitle = "Expanded Scale for Predictable Anchor and Minor Character Action-DN Clusters",
    x     = "Accumulated Action Volume (Plot Mechanics)",
    y     = "Accumulated Description by Narrator Volume (Narratorial Scrutiny)"
  ) +
  theme_manuscript() + 
...
output_zoomed <- file.path(home_dir, "FIGURE_1_6_DN_TOPOLOGY_MAP_ZOOMED.png")
ggsave(output_zoomed, plot = fig1_6_act_dn_zoomed, width = 10, height = 7, units = "in", dpi = 300)


message("---")
message(paste("Success! Full Scale Map saved to:", output_full))
message(paste("Success! Strictly Bounded Zoom Profile (x <= 35, y <= 12) saved to:", output_zoomed))
message("---")
